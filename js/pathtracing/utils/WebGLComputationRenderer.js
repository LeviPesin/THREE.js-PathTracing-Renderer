...
import {int, vec2, pixel, add, mul} from 'three-nodes/Nodes.js';
...
export default class WebGLComputationRenderer {
	...
	setBuffers(srcBuffer, outBuffer) {
		...
		const pixelNode = pixel(vec2(outBuffer.width, outBuffer.height));
		const index = add(mul(int(pixelNode.y), int(outBuffer.width)), int(pixelNode.x));
