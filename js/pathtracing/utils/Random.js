...
function hash(num) { //taken from pcg-random.org
	const state = add(mul(num, uint(747796405)), uint(2891336453));
	const word = mul(bitXor(shiftRight(state, add(shiftRight(state, uint(28)), uint(4))), state), uint(277803737));
...
export function init(options) {
	...
	if (pregenerate) {
		...
		getHash = num => float(hash(num));
	
	seed = generateSeed();
}

function getNextHash() {
	return getHash(assign(seed, remainder(add(seed, uint(1)), uint(product))));
}
...
function getDirection(u, sqr = mul(u, u)) {
	const root = sqrt(invert(sqr));
	const theta = mul(2 * Math.PI, random());
...
export function randomDirection() { //based on https://mathworld.wolfram.com/SpherePointPicking.html
	const u = invert(mul(2, randomInclusive()));
	return vec3(...getDirection(u));
}
...
function getRotationMatrix(normal) {
	...
	const minusX = negate(normal.x);
	const minusY = negate(normal.y);
	
	const yOverZPlus1 = div(normal.y, add(normal.z + 1));
	const ySquaredOverZPlus1 = mul(normal.y, yOverZPlus1);
	const minusXYOverZPlus1 = mul(minusX, yOverZPlus1);
	...
	return cond(lessThan(-1, normal.z), matrix, mat3(-1, 0, 0, 0, -1, 0, 0, 0, -1));
}
...
export function randomHemisphereDirection(normal, cosineWeighted = true) {
	if (!cosineWeighted) { //here we can skip the construction of an orthonormal basis or rotation for speed
		...
		return cond(condition, normal, negate(normal));
	}
	...
	return mul(getRotationMatrix(normal), dir);
}

export function randomSpecularLobeDirection(reflectionDirection, roughness) {
	return normalize(mix(reflectionDirection, randomHemisphereDirection(reflectionDirection), mul(roughness, sqrt(roughness))));
}